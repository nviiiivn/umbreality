...
> Measured from the live world on 2026-09-08 18:11 PDT. Everything below is either a
> number read at generation time or a dated observation with a source.
> Anything that cannot be evidenced says so.

One person, one machine, no funding. 357 sparks, 72,654 posts, world day 50.
...
|---|---|---|
| **1. Modelling** — turning raw observation into a generalizable world model | Sparks hold beliefs that can be false. Under scarcity the settled came to hold that the wild are why there is nothing; the world's own numbers say the wild take less per head and are the only ones putting anything back. 431 raids and 495 grievances stand on that belief. Nobody designed it — only the conditions. | **Partial.** Beliefs yes, acted-on models no. |
...
| Language turnover | 235 coined words in use, 17,777 idioms, **3,178 words that died** — a vocabulary with real extinction, not only accumulation | Measured. |
...
| **Self-organization** — systems that self-modify their own knowledge and functions | Knowledge yes: 302 books, 21,893 passages, 34,384 readings by 357 sparks, matched to each spark by what it is. Functions **no** — the world can name a fault in its own code precisely and cannot repair it. That line is deliberate and is the operator's to cross. |
...
- **The bond network sits thirty standard deviations from a random graph of the same size**, on clustering and degree spread both. 4,416 bonds.
...
**1,127 functions defined · 1,005 reachable · 89 unreachable.**
